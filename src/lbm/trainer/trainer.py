...
class TrainingPipeline(pl.LightningModule):
    ...
    def training_step(self, train_batch: Dict[str, Any], batch_idx: int) -> dict:
        ...
        if (
                "adapter_residual_norm" in model_output
                and self.global_step % 10 == 0
        ):
            msg = (
                f"[ADAPTER DEBUG] step={self.global_step} "
                f"mean={model_output['adapter_residual_norm'].detach().float().item():.8e}"
            )

            for i in range(4):
                k = f"adapter_down{i}_norm"
                if k in model_output:
                    msg += f" down{i}={model_output[k].detach().float().item():.8e}"

            # Print learnable gates if available.
            if hasattr(self.model, "face_adapter") and self.model.face_adapter is not None:
                fa = self.model.face_adapter
                if hasattr(fa, "down_gates"):
                    gates = fa.down_gates.detach().float().cpu().tolist()
                    msg += " gates=[" + ", ".join(f"{g:.4f}" for g in gates) + "]"

                if hasattr(fa, "mid_gate") and fa.mid_gate is not None:
                    msg += f" mid_gate={fa.mid_gate.detach().float().item():.4f}"
